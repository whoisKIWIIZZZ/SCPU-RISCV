...
//Counter read or write & set		counter_ch=SC1 SC0; counter_Ctrl=XX M2 M1 M0 X
	...
	always @ (posedge clk or posedge rst) begin 
		if (rst )
			begin //counter0_Lock <=0;counter1_Lock <=0;counter2_Lock <=0;counter_Ctrl<=0;
			counter0_Lock <= 32'hAFFFF;  // 复位时给最大初值
        counter1_Lock <= 32'hFFFFFFFF;
        counter2_Lock <= 32'hFFFFFFFF;
        counter_Ctrl  <= 24'h000002;    // 直接复位成模式01（自动重载）
		counter0 <= 32'hFFFF;
        M0 <= 1'b1;                     // 复位时触发一次加载
        M1 <= 1'b0;
        M2 <= 1'b0;
...
// Counter channel 2	
	...
	// 	endcase
	assign counter_out = (counter_ch == 2'h0) ? counter0[31:0] :
                     (counter_ch == 2'h1) ? counter1[31:0] :
                     (counter_ch == 2'h2) ? counter2[31:0] :
                     {8'h00, counter_Ctrl};
